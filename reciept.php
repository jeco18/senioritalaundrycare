...
function getItemsReciept(){
    global $db;
    global $id;
    $sql = "SELECT e.qty, e.item_id, i.name, i.price
        FROM items AS i
    ...
    foreach($result as $row){
        ...
        <tr class="font-monospace">
                    <td class="font-monospace text-start mt-0 mb-0" style="font-size: 10px;"><?= $row['name']?></td>
                    <th class="font-monospace text-center mt-0 mb-0" style="font-size: 10px;"><?= $row['price']?></th>
...
$sql = "SELECT SUM(e.qty * i.price) AS items_total
...
$stmt->bindParam(':id', $id);
$stmt->execute();
$items_total_result = $stmt->fetch(PDO::FETCH_ASSOC);
$items_total = $items_total_result['items_total'];

$total = $laundry_total + $items_total; // Calculate overall total
...
<head>
    ...
    <title>Reciept - Seniorita Laundry Care</title>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <img src="assets/img/washing-clothes.gif" width="40">&nbsp;Seniorita Laundry Care<br>
                    <span style="font-weight: normal !important;">Alburo St. Salog, Sorsogon City</span><br>
                    <span style="font-weight: normal !important;">Phone (+63) 930-262-3263</span><br>
